...
const addDefinePlugin = `new (require("webpack")).DefinePlugin({ 'process.type': 'renderer', 'process.env': {NODE_ENV:JSON.stringify(process.env.NODE_ENV||'development')}}),`;

async function patchAngular() {
  ...
  browserResult = browserResult.replace(
    /node: false,/g,
  ...
  browserResult = browserResult.replace(
    /plugins: extraPlugins.concat\(\[/g,
    `plugins: extraPlugins.concat([${addDefinePlugin}`,
  );
